...
.SYNOPSIS
    ...
    Determine whether a target(s) smb server has SMB signing required or not. Useful for determining targets for SMB relay attacks. 
    SMB functions borrowed from Invoke-InveighRelay:
    https://github.com/Kevin-Robertson/Inveigh/blob/master/Scripts/Inveigh-Relay.ps1
    TODO: add pipe input support

.PARAMETER Target

    Target SMB server to check for SMB signing requirements.

.PARAMETER Targets

    Specify multiple servers to check.

.PARAMETER Delay

    Specifies the interval to wait between checks for multiple servers.

.PARAMETER DelayJitter

    Specifies the maximum amount of randomized time to add to the delay. 

.EXAMPLE

    Get-SMBSigning -Targets @("test1","test2") -Delay 4 -DelayJitter 10

.EXAMPLE

    Get-SMBSigning -Target "computer01"
...
Param (
    ...
    foreach ($Target in $Targets) {
        ...
        if ($SigningStatus){
            "SMB signing is required on $Target"
        } else {
            "SMB signing is not required on $Target"
        }
        if ($Delay) {
